...
%NSE cal
T_cal_obs_mean = mean(T_cal(:,5),"omitnan");
nse_cal_numer = sum((T_cal(:,5)-T_cal(:,6)).^2, "omitnan");
...
NSE_val= 1 - (nse_val_numer/nse_val_denom);
...
title(['Calibration, RMSE=' num2str(RMSE_cal) '°C NSE=' num2str(NSE_cal)]);
...
xlabel('Time');
ylabel('Temperature [°C]');
title(['Validation, RMSE=' num2str(RMSE_val) '°C NSE=' num2str(NSE_val)]);
legend('Air temperature','Observed water temperature','Simulated water temperature','location','SouthEast')
datetick('x','mmm-yy')
set(gcf,'paperunits','centimeters','papersize',[18 10],'paperposition',[0 0 18 10]);
